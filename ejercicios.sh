...
echo "== Instalando Apache2 y herramientas necesarias =="
sudo apt update
sudo apt install -y apache2 openssl apache2-utils

echo "== Habilitando y verificando el servicio Apache2 =="
sudo systemctl enable apache2
sudo systemctl start apache2
sudo systemctl status apache2

echo "== Modificando página inicial de Apache2 =="
echo "<h1>Página modificada del servidor</h1>" | sudo tee /var/www/html/index.html

echo "== Configurando Hosts Virtuales =="
# Crear directorios raíz para los hosts virtuales
sudo mkdir -p /var/www/$DOMAIN1 /var/www/$DOMAIN2
echo "<h1>Inicio de $DOMAIN1</h1>" | sudo tee /var/www/$DOMAIN1/index.html
echo "<h1>Inicio de $DOMAIN2</h1>" | sudo tee /var/www/$DOMAIN2/index.html

# Cambiar propietario
sudo chown -R www-data:www-data /var/www/$DOMAIN1 /var/www/$DOMAIN2

# Configurar archivos de hosts virtuales
...
# Modificar /etc/hosts para pruebas locales
echo "== Configurando /etc/hosts para los dominios =="
sudo tee -a /etc/hosts <<EOF
127.0.0.1 $DOMAIN1
127.0.0.1 $DOMAIN2
EOF

echo "== Configurando acceso restringido para $DOMAIN1 =="
# Crear directorio restringido
...
# Configurar .htaccess para proteger el directorio
...
# Permitir .htaccess en Apache2
sudo sed -i '/<Directory \/var\/www\/>/,/<\/Directory>/s/AllowOverride None/AllowOverride All/' /etc/apache2/apache2.conf
sudo systemctl reload apache2

echo "== Generando certificado SSL para $SECURE_DOMAIN =="
# Crear directorio raíz para el host seguro
...
# Modificar /etc/hosts para el dominio seguro
echo "== Configurando /etc/hosts para el dominio seguro =="
sudo tee -a /etc/hosts <<EOF
127.0.0.1 $SECURE_DOMAIN
EOF

# Reiniciar Apache2 para aplicar cambios
sudo systemctl reload apache2
